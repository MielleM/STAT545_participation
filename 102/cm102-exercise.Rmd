...
There's that famous sentence about the quick brown fox that contains all letters of the alphabet, although I don't quite remember the sentence.Show in New WindowClear OutputExpand/Collapse Output
...
> . is a single wildcard in regular expressions
...
str_subset(countries, pattern = "a.a.a") #actually gives us a the vector 
```
...
>To do without spaces, added spaces inside square brackets
...
```{r}
# parenthesis are like math, does what's in the parenthesis first and then the rest 
c("bear", "beer", "bar") %>% 
    str_view_all_match(pattern = "be(e|a)r")
```
```{r}
#give me e OR a or neither?
c("bear", "beer", "bar", "bee", "nice") %>% 
