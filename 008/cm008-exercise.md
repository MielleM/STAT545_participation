...
``` r
# if add assignment operator, then the new column with appear in the gapminder df (mygap <-)
gapminder %>% 
...
``` r
# first argument is what to select, second argument of if_else function will be applied if True (70), third argument if False (lifeExp)
...
gapminder %>% 
  group_by(continent) %>% 
  # summarise(num = n()) ... tally() or count() does same thing
  tally()
```
...
``` r
# use lag() function to calculate change from previous entry to next entry
gapminder %>% 
  group_by(country) %>% 
...
-   `filter()`
-   `arrange()`
-   `mutate()` \#vectorize computations
-   `summarize()`\#reduces down to 1

also groupby() count(), tally() are the same thing
