...
> setNames() takes vector or list and makes it the column names of another one 
...
>using map logical `map_lgl`, subset list column of tibble to select any entries that include strawberry
>function is NOT vectorized, so have to use purrr 
> .x is a vector of ice cream flavors
> returns vector of T/F for strawberry ice cream
...
```{r}
str(got_alias) #contains null values 

got_alias %>% 
  drop_na() %>% # removes null or NA values
  unnest()
```
