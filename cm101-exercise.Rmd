...
# square <- function(x) {
#  y <- x^2 
#  return(y) #r will stop what it's doing and return y, if you want to stop the function early (it won't return anything after return())
#  }
...
> expect_something functions in testthat package. Errors if test doesn't pass, returns nothing if it does pass. 
> expect_equal tests if two arguments are equal. If test passes, doesn't return anything. 
> expect_idential is stricter 
> all arguments separated by a comma
> expect_true, expect_less...than? 
...
> test_that("name of test that's readable to you", {the test itself})
> test_that is more self-documenting
...
>stopifnot() creates double negatives (stopifnot(!is.numeric(x))) means stop if the function is not numeric 
...
# replacing default probabilities (probs()) with your own inputs (defaults were 0, 1)
...
> quantile is just an estimate, there are multiple ways to calculate it 
> type lets you input which method for estimating quantiles 
> the `...` means that whatever goes there, will be put into other occurences of `...`
