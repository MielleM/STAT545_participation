...
``` r
full_join(a, b) #dplyr found a common column between the two datasets (using column name)
```
...
``` r
#take x1 column from a dataset, x3 column from b dataset -- there are no matches, so printed all NA
...
``` r
setdiff(y, z) # row in y that isn't in z
```
...
``` r
setdiff(z, y) #row in z that isn't in y
```
...
``` r
#columns MUST have identical columns for intersect and union to work 
# intersect(y, x) 
# union(y, x)
```
...
``` r
# Does not match rows based on common column entries, only on column names by order of rows. 
bind_cols(y, z)
```
...
``` r
#duplicate entries will duplicate values from other columns 
```
