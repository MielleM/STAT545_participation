...
Vectors: 
> atomic/homogenous-- can only hold 1 data type (simple stuff like characters, not functions)
...
> list is a train, each car holds stuff (vector, function, etc, can be different)
> [1] is for car 1, regardless of what's in the car
> [[1]] is for contents of car 1
> $ extract components of named car
...
```{r}
# loops automatically (will square each one)
(1:10) ^ 2
```

```{r}
# shorter vector will be recycled to finish out longer vector
(1:10) * (1:2)
```
...
```{r}
str(wesanderson)
# vectors of lengths 1:4 or 1:5, with hex colors

# for loop example (code garbage basically)
x <- character(0)
...
#max.level is how many lists of lists do you want to see? 
```
...
> map_type functions (dbl (decimal), integer, chr, lgl for logical)
...
```{r}
# Method 1
vec1 <- function(x) x[1] #function that subsets anything to its first component
map_chr(wesanderson, vec1)

# Method 2: head 
map_chr(wesanderson, head, n=1)

# Method 3: because x[1] is actually the function `[`(x, 1) under the hood
...
map_dfr(got_chars, `[`, desired_info) #put it in a data frame! now we can use it 
```
